...
test("Board changes when imitating click", () => {
  ...
  const { rerender } = render(
    <Board board={initialBoard} click={(index) => onClickBox(index)} />
  );
  ...
  rerender(
    <Board board={boardAfterClick} click={(index) => onClickBox(index)} />
  );
...
test("UserList updates when new player joins", () => {
  ...
  const { rerender } = render(
    <UsersList players={initialPlayers} spectators={initialSpectators} />
  );
  ...
  rerender(
    <UsersList players={playersAfterJoin} spectators={spectatorsAfterJoin} />
  );
  expect(screen.getByText("bob")).toBeInTheDocument();
...
test("Current user is highlighted in leaderboard", () => {
  const username = "joe";
  const leaderboard = [
    { score: 101, username: "joe" },
    { score: 100, username: "thomas" },
    { score: 99, username: "bob" },
  ];
